...
class process_request(GlooLLMTaskInterface):
    ...
    def override_prompt(self) -> str:
        ...
        return f"""
        Extract the information from the INPUT text in the OUTPUT format described. For the "status" field, use the following definitions:
            ...
            **{
                ...
                "ClassifyRequestOutput": {
                    "trackingNumber": {
                        ...
                        "definition": "optional string, Any tracking number or identifier the agency has assigned to the request in the response.  This may be blank if not present.",
                    },
                    "dateEstimate": {
                        "alias": "dateEstimate",
                        "definition": "optional string, Any estimated date of completion supplied by the agency in the response, formatted as YYYY-MM-DD.",
                    },
                    "price": {
                        "alias": "price",
                        "definition": "integer, The price for any fee that the agency requires to complete the request. Write -1 if not present.",
                    },
                    "reasoning": {
                        "alias": "reasoning",
                        "definition": "Reasoning for why request may be in a certain status.",
                    },
                    "status": {
                        "alias": "status",
                        "definition": f"{VARS.out_Status.desc}",
                    },
                },
            }
        )
...
async def run_process_request_v0_async(
...
def run_process_request_v0_sync(
    _in: ClassifyRequestInputModel, **kwargs
) -> ClassifyRequestOutputModel:
    return asyncio.run(run_process_request_v0_async(_in, **kwargs))


# Only the run_v0 function is directed exported.
__all__ = [
    "run_process_request_v0_async",
    "run_process_request_v0_sync",
